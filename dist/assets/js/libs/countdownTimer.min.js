...
(function($) {
    ...
    function html($this, time, opts) {
        ...
        if (opts.labelsFormat === true && typeof window['regexpMatchFormat_' + $this.attr('id')] === 'undefined' &&
            ...
            for (var a = time.length; a > 0; a--) {
                ...
                var word = labelarr[itr](+time[itr]).toLowerCase();
                labelTime += "<span class='displaySection'><span class='numberDisplay'>" + time[itr] + "</span><span class='periodDisplay'>" + word + "</span></span>";
            }
            time = labelTime += "</span>";
    ...
    $.fn.countdowntimer.regionalOptions = {
        labels: [
	        Typo.pluralize(["Год", "Года", "Лет"]),
        	Typo.pluralize(["Месяц", "Месяца", "Месяцев"]),
        	Typo.pluralize(["День", "Дня", "Дней"]),
        	Typo.pluralize(["Час", "Часа", "Часов"]),
        	Typo.pluralize(["Минута", "Минуты", "Минут"]),
        	Typo.pluralize(["Секунда", "Секунды", "Секунд"]),
	        ],
